...
public class Student {
	...
	public static Optional<Student> create(final String firstName, final String lastName, final int indexNumber) {
		String insertSql = String.format("INSERT INTO %s (first_name, last_name, index_number) " +
						"VALUES ('%s', '%s', '%d');", TABLE_NAME, firstName, lastName, indexNumber);
	...
	public static Optional<Student> findByIndexNumber(final int indexNumber) {
		...
		Optional<Student> student = Optional.empty();
		try {
			ResultSet read = QueryExecutor.read(findByIndexNumberSql);
			int id = read.getInt("id");
			String firstName = read.getString("first_name");
			String lastName = read.getString("last_name");
			student = Optional.of(new Student(id, firstName, lastName, indexNumber));
		} catch (SQLException e) {
			System.out.println(e.getMessage());
		}
